...
var Heroku = function(token, baseURL) {
  ...
  this.apps = function(callback) {
    req.get(this.buildURL('apps'), this.handleResponse(callback));
  }

  this.buildURL = function(path) {
    return 'https://:' + this.token + '@' + this.baseURL + '/' + path;
  }
    ...
    return function(err, res, body) {
      if (!err && res.statusCode == 200) {
        ...
        if(err) {
          ...
          callback(new Error('Request failed with response status: ' + res.statusCode), body);
        }
      }
    }
  }
